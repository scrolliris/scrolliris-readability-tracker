...
class Client {
  ...
  _checkConfig() {
    let error = false;
    if (typeof this.publicationKey === 'undefined' ||
        this.publicationKey === null || this.publicationKey === '') {
      error = true;
  ...
  _genTraceKey(cookieKey='t') {
    let k = Cookie.read(cookieKey);
    if (typeof k === 'undefined' || k === null) {
      k = Base64.urlsafe_b64encode(UUID.uuid4()).replace(/\=\=$/, '');
      // set value to cookie (45 minutes, t=...)
      Cookie.write(cookieKey, k, 45);
    }
    k += '--';
    return k;
  }
